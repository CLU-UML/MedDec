...
def load_diff():
    df = pd.read_csv('/data/mohamed/nidhi_mimic/annotation_umls_count_fixed.csv')
...
def compute_diff():
    ...
    print(df)
    df.to_csv('/data/mohamed/nidhi_mimic/annotation_umls_count_fixed.csv', index=False)


def fix_df():
    df = pd.read_csv('/data/mohamed/nidhi_mimic/annotation_umls_count.csv')

    data_dir = '/data/mohamed/data/mimic_decisions/data'
    starts, ends, lens = [], [], []
    for i, row in tqdm(df.iterrows(), total=len(df)):
        dec = row['decision']
        fn = row['span_discharge_summary_id']
        annots_file = glob(f'{data_dir}/*/{fn}*')[0]
